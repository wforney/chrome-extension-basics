 ...
 * Modified for TypeScript by William Forney.
 * @author Keith Henry <keith.henry@evolutionjobs.co.uk>
 * @author William Forney <bill@billforney.com>
 * @license MIT
 */
(() => {
    "use strict";
     ...
     */
    function promisify(f: any, parseCB: (arg0?: any) => void) {
        return (...args: any[]) => {
            let safeArgs = args;
            let callback: any;
            ...
            return new Promise((resolve, reject) => {
                try {
                    ...
                    f(...safeArgs, (...cbArgs: any[]) => {
                        ...
                        // Chrome extensions always fire the callback, but populate
                        // chrome.runtime.lastError with exception details
     ...
     */
    function applyMap(api: any, apiMap: any[]) {
        if (!api) {
     ...
     */
    function applyMaps(apiMaps: { [key: string]: any }) {
        for (const apiName of Object.keys(apiMaps)) {
            const callbackApi: any = (chrome as any)[apiName];
            if (!callbackApi) {
                // Not supported by current permissions
                continue;
            }
    ...
    applyMaps({
        ...
        enterprise: [
            { n: "platformKeys", props: ["getToken", "getCertificates", "importCertificate", "removeCertificate"] },
        ],
        ...
        platformKeys: ["selectClientCertificates", "verifyTLSServerCertificate",
            { n: "getKeyPair", cb: (publicKey: any, privateKey: any) => ({ publicKey, privateKey }) }],
        runtime: [
            ...
            { n: "requestUpdateCheck", cb: (status: any, details: any) => ({ status, details }) }],
